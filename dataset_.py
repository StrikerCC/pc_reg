...
#         self.voxel_sizes = (0.67,)
#         # self.voxel_sizes = np.arange(self.voxel_sizes[0], self.voxel_sizes[1], (self.voxel_sizes[1]-self.voxel_sizes[0])/4)
#
#         self.angles_cutoff_along = (90.0, 180.0)
#         # self.angles_cutoff_along = np.arange(self.angles_cutoff_along[0], self.angles_cutoff_along[1], 90)
#         self.angle_cutoff = 90
#
#         self.plane_sizes = (1.2, 1.5)
#         # self.plane_sizes = np.arange(self.plane_sizes[0], self.plane_sizes[1], 3.0)
#
...
#                           'happy_recon'] # , 'lucy']
#         self.data_info = {'pc_model': None, 'pc_artificial': None, 'instance': None, 'unit': None, 'voxel_size':  None,
#                           'angle': None, 'pose': None, 'sigma': None, 'outliers': None, 'plane': None}
#
...
#
#         # operate pcs
#         sources_ins = self.load_pc(self.file_paths, self.instances)[1]
#         sources_ins = [sources_ins]
#         sources_record = []
#
#         for i, source_ins in enumerate(sources_ins):
#             print(i, 'Creating artificial point cloud for', source_ins['instance'])
#             sources = self.down_sampling([source_ins], flag_show=False)
...
#             sources_record += sources
#             print(i, 'Created artificial point cloud for', source_ins['instance'], '\n')
#
...
#             filename = '0' * (filename_len - len(str(index))) + filename
#             pc = source['pc_artificial']
#             o3.io.write_point_cloud(save_dir + filename, pc)
#             del source['pc_artificial']
#             del pc
#             source['pc_artificial'] = save_dir + filename
#
#     def execute(self, source):
#         pass
#
#     def down_sampling(self, sources, flag_show=True):
#         print('Down sampling', len(sources), 'sources')
#         sources_processed = []
...
#         print('Down sampling done, get', sources[-1]['pc_artificial'], 'bounded by', sources[-1]['pc_artificial'].get_max_bound(), sources[-1]['pc_artificial'].get_min_bound())
#         return sources_processed
#
#     def cutoff(self, sources, flag_show=True):
#         print('cutting off', len(sources), 'sources')
#         sources_processed = []
#         for source in sources:
#             pc = source['pc_artificial']
#             tp = np.asarray(pc.points)
...
#         print('cutting off done')
#         return sources_processed
#
#     def add_pose(self, sources, flag_show=False):
#         print('Adding pose to', len(sources), 'sources')
#         sources_processed = []
#         for source in sources:
#             pc = source['pc_artificial']
#             tp = np.asarray(pc.points)
...
#                 source_ = copy.deepcopy(source)
#                 pc_ = source_['pc_artificial']
#                 translation = rg * ((self.translation_rg_factor[1] - self.translation_rg_factor[0]) * np.random.random((3)) + self.translation_rg_factor[0])
#                 orientation = (self.rotation_reg[1] - self.rotation_reg[0]) * np.random.random((3, 1)) + self.rotation_reg[0]
#                 tf = np.identity(4)
#                 tf[:3, :3] = t3d.euler.euler2mat(*orientation)
#                 tf[:3, 3] = translation
#                 pc_.transform(tf)
#
#                 assert isinstance(source_['pose'], np.ndarray)
#                 source_['pose'] = np.matmul(tf, source_['pose'])
#                 # source_['pose'] = np.matmul(source_['pose'], tf)
#                 source_['pose'] = tf.tolist()
#                 sources_processed.append(source_)
#                 if flag_show:
#                     tf = np.eye(4)
#                     tf[:3, 3] = pc_.get_max_bound()
#                     draw_registration_result(source=pc, target=pc_, transformation=tf,
#                                              window_name='Move at ' + str(tf))
#         print('Adding pose done')
#         return sources_processed
#
#     def add_outliers(self, sources, flag_show=True):
#         print('Adding outliers to', len(sources), 'sources')
#         sources_processed = []
#         for source in sources:
#             pc = source['pc_artificial']
#             tp = np.asarray(pc.points)
...
#         print('Adding outliers done')
#         return sources_processed
#
#     def add_plane(self, sources, flag_show=True):
#         print('Adding background plane to', len(sources), 'sources')
#         sources_processed = []
#         for source in sources:
#             pc = source['pc_artificial']
#             tp = np.asarray(pc.points)
...
#         print('Adding background done')
#         return sources_processed
#
#     def add_noise(self, sources, flag_show=True):
#         print('Adding Gaussian noise to', len(sources), 'sources')
#         sources_processed = []
#         for i, source in enumerate(sources):
#             pc = source['pc_artificial']
#             tp = np.asarray(pc.points)
...
#         bunny_data = dl[i]
#         pc_model = bunny_data['pc_model']
#         pc_artificial = bunny_data['pc_artificial']
#         tf = bunny_data['pose']
